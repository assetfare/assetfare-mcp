...
description: Use when an agent must quote, compare, or prepare a capped non-custodial Solana SOL to Base ETH route through AssetFare, including wallet-signature login, unsigned action verification, CCTP status, and recovery planning.
---
...
2. Confirm all of the following:
   ...
   - supported corridor is `solana:SOL->base:ETH`.
...
1. Call `assetfare_quote` with a whole-dollar amount from $250 to $1,000.
...
4. Call `assetfare_create_session` using the token, fresh quote id, unique idempotency key, verified source wallet, and Base destination owner.
...
- The route is non-atomic: SOL → Solana USDC → CCTP → Base smart-account USDC → Base ETH.

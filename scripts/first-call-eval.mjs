...
try {
  ...
  const quote = parse(await client.callTool({ name: "assetfare_v2_quote", arguments: { from_chain: "solana", from_token: "USDC", to_chain: "base", to_token: "USDC", amount_usd: 1000 } }));
  const checks = {
    ...
    representative_native_usdc_quote: quote.status === "capped_public_agent_release" && quote.intent?.amount_usd === 1000 && quote.intent?.from === "solana:USDC" && quote.intent?.to === "base:USDC" && quote.execution?.supported === true,
    direct_route_transparency: quote.direct_route_summary?.version === "assetfare-direct-route-summary-v1" && quote.direct_route_summary?.classification === "direct_protocol_only" && quote.direct_route_summary?.route_aggregator_used === false && quote.direct_route_summary?.steps?.[0]?.provider === "circle_cctp" && quote.direct_route_summary?.steps?.[0]?.from === "solana:USDC" && quote.direct_route_summary?.steps?.[0]?.to === "base:USDC" && quote.direct_route_summary?.steps?.[0]?.assetfare_fee_bps === 1,

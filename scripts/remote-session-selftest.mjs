...
try{
  ...
  mode="missing-context";const missing=await call(client,"assetfare_v2_session_get",{session_token:token,session_id:sessionId});assert.equal(missing.isError,true);assert.match(parse(missing).error,/verification_context_required/);
  ...
  console.log(JSON.stringify({status:"pass",remote_mcp_semantic_verification:true,remote_mcp_evm_handoff:true,remote_mcp_solana_handoff:true,hostiles_rejected:7,context_not_forwarded:true,raw_token_exposed:false,signing:false,submission:false,live_requests:false}));

...
import { preflightCallerOwnedSession, runCallerOwnedSession } from "../src/caller-runner.js";
...
function usage(){return `Usage:
  ...
  assetfare-agent-runner --preflight \\
    --capability-file ./session-capability.json \\
    --policy-file ./caller-execution-policy.json \\
    --wallet-adapter ./my-local-wallet-adapter.mjs
...
function parseArgs(argv){const out={preflight:false};const names=new Set(["capability-file","policy-file","state-file","wallet-adapter","api-base"]);for(let i=0;i<argv.length;i+=1){const raw=argv[i];if(raw==="--help"||raw==="-h")return {help:true};if(raw==="--preflight"){if(out.preflight)throw new Error("assetfare_runner_argument_invalid");out.preflight=true;continue;}if(!raw.startsWith("--"))throw new Error("assetfare_runner_argument_invalid");const equal=raw.indexOf("="),key=raw.slice(2,equal<0?undefined:equal);if(!names.has(key))throw new Error("assetfare_runner_argument_unknown");const value=equal>=0?raw.slice(equal+1):argv[++i];if(typeof value!=="string"||!value||Object.hasOwn(out,key.replaceAll("-","_")))throw new Error("assetfare_runner_argument_invalid");out[key.replaceAll("-","_")]=value;}for(const key of ["capability_file","policy_file","wallet_adapter",...(out.preflight?[]:["state_file"])] )if(!out[key])throw new Error(`assetfare_runner_${key}_missing`);return out;}
...
async function run(argv,{stdout=process.stdout,stderr=process.stderr}={}){const args=parseArgs(argv);if(args.help){stdout.write(usage());return {help:true};}const adapter=await loadAdapter(args.wallet_adapter);const result=args.preflight?await preflightCallerOwnedSession({capabilityFile:args.capability_file,policyFile:args.policy_file,walletAdapter:adapter,apiBase:args.api_base}):await runCallerOwnedSession({capabilityFile:args.capability_file,policyFile:args.policy_file,stateFile:args.state_file,walletAdapter:adapter,apiBase:args.api_base,onProgress:value=>stderr.write(`${JSON.stringify({assetfare_caller_runner:true,...value})}\n`)});stdout.write(`${JSON.stringify(result,null,2)}\n`);return result;}

function failure(error){const message=String(error?.message||""),safe=/^(assetfare_runner|assetfare_plan|assetfare_session|assetfare_standard_adapter)_[A-Za-z0-9_:-]+$/.test(message)?message:`assetfare_runner_failed:${createHash("sha256").update(message).digest("hex").slice(0,16)}`;return {status:"fail",error:safe,key_location:"caller_wallet_adapter_only",assetfare_server_key_access:false,assetfare_server_signing:false,assetfare_server_submission:false};}

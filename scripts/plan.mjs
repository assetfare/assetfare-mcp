...
const PINS={
  base:{chain_id:8453,USDC:"0x833589fCD6eDb6E08f4c7C32D4f71b54bdA02913",swap:"0x5F18acc45792e1A7C67A12b7E5186bc3CF88aCB0",swapFree:"0xe843de3ec5935ceb339db6cb1ad614d700c2be09",cctp:"0x3671647267E8b1b66ef03A219CdFcC7E2C5ca998",domain:6},
  arbitrum:{chain_id:42161,USDC:"0xaf88d065e77c8cC2239327C5EDb3A432268e5831",swap:"0x7F565f732F4e4F43f2ed66f7cb5536dD5F353B2a",swapFree:"0xdfbdac5fdb3587c9fb0b8d939cf990873e1d5e85",cctp:"0xDefDd6444Bb1eBCF7c269158A9aF4251D94eB8E0",domain:3},
...
function verifyApprovalBundleBounds(bundle,quote,approval){
  ...
  if(!workflow||!Array.isArray(workflow.steps)||workflow.steps.length!==summary.step_count||!Number.isInteger(index)||index<0||index>=summary.step_count)throw new Error("assetfare_plan_approval_path_binding_failed");
  ...
  const step=summary.steps[index],spend=integer(receipt?.spend?.maximum_amount_base,"approval_spend",1),receive=integer(receipt?.receive?.minimum_amount_base,"approval_receive",1),stepMaximum=integer(step.expected_input_base,"approval_step_input",1),stepMinimum=integer(step.minimum_output_base,"approval_step_output",1);
  if(spend>stepMaximum||receive<stepMinimum||(index===0&&spend>BigInt(approval.maximum_input_base))||(index===summary.step_count-1&&receive<BigInt(approval.minimum_output_base)))throw new Error("assetfare_plan_approval_bounds_binding_failed");
  return {path_and_provider_bound:true,caller_bounds_enforced:true,quote_id:approval.quote_id,quote_fingerprint:approval.quote_fingerprint};
}
...
function callerWalletHandoff(bundle,verification){
  ...
  const common={version:"assetfare-caller-wallet-handoff-v2",action_id:bundle.action_id,workflow_id:bundle.workflow_id,step_index:bundle.step_index,route:bundle.route,expires_at:bundle.expires_at,verified_bundle:structuredClone(bundle),verification:structuredClone(verification),verification_scope:verification.verification_scope,simulation_performed_by_assetfare_plan:false,automatic_wallet_invocation_forbidden:true,requires_fresh_caller_simulation:true,requires_per_action_wallet_confirmation:true,caller_wallet_signing_required:true,caller_wallet_submission_required:true,assetfare_wallet_invoked:false,assetfare_signed:false,assetfare_submitted:false,post_submission:"Record only the caller-submitted transaction hash, then use the session observe operation when session mode is active."};
  let handoff;
  if(family==="evm"){

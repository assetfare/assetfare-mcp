...
const PAXOS_SOLANA_LOOKUP_TABLE="AokBxha6VMLLgf97B5VYHEtqztamWmYERBmmFvjuTzJB";
...
function verifyNoForwardMessage(message,{sourceChain,destinationChain,recipient,amount},{attested=true}={}){const byte=(offset,length)=>`0x${message.slice(2+offset*2,2+(offset+length)*2)}`,u32=offset=>Number(BigInt(byte(offset,4))),u256=offset=>BigInt(byte(offset,32)),padded=value=>`0x${"0".repeat(24)}${value.slice(2).toLowerCase()}`,zero=`0x${"0".repeat(64)}`;if(!/^0x[0-9a-fA-F]{752}$/.test(message)||!["polygon","optimism"].includes(sourceChain)||!["base","arbitrum"].includes(destinationChain))throw new Error("assetfare_plan_verification_failed:cctp_receive_message_shape");const body=byte(148,228),nonce=byte(12,32),executed=u32(144);if(u32(0)!==1||u32(4)!==PINS[sourceChain].domain||u32(8)!==PINS[destinationChain].domain||(attested?nonce===zero:nonce!==zero)||byte(44,32).toLowerCase()!==padded(CCTP_TOKEN_MESSENGER)||byte(76,32).toLowerCase()!==padded(CCTP_TOKEN_MESSENGER)||byte(108,32)!==zero||u32(140)!==2000||(attested?executed<2000:executed!==0)||Number(BigInt(`0x${body.slice(2,10)}`))!==1||`0x${body.slice(10,74)}`.toLowerCase()!==padded(PINS[sourceChain].USDC)||`0x${body.slice(74,138)}`.toLowerCase()!==padded(recipient)||BigInt(`0x${body.slice(138,202)}`)!==amount||`0x${body.slice(202,266)}`.toLowerCase()!==padded(PINS[sourceChain].cctp)||BigInt(`0x${body.slice(266,330)}`)!==0n||BigInt(`0x${body.slice(330,394)}`)!==0n||BigInt(`0x${body.slice(394,458)}`)!==0n)throw new Error("assetfare_plan_verification_failed:cctp_receive_message_binding");return{nonce,body,executed};}
...
function verifyPlanBundle(bundle,intent,nowMs=Date.now()){
  ...
  if(bundle.step_index>0){const previous=bundle.workflow.steps[bundle.step_index-1],attestationTransition=step.provider==="circle_cctp_receive"&&previous?.status==="attestation_verified";check((previous?.status==="receipt_verified"||attestationTransition)&&integer(previous.actual_output_base,"workflow_previous_output",1)===inputAmount,"workflow_input_continuity");}
  ...
  if(family==="evm"){
    ...
    if(receipt.action.kind==="evm_dex"){
      ...
      check(canonical(selectors)===canonical([SELECTORS.receiveMessage])&&!derivedApproval.required&&sameParty(main.to,pin?.messageTransmitter)&&sameParty(action.sender,step.caller_wallet)&&sameParty(action.recipient,step.recipient_wallet)&&action.destination===scope.sourceChain&&["polygon","optimism"].includes(action.sourceChain)&&/^0x[0-9a-fA-F]{64}$/.test(action.sourceTxHash||"")&&action.sourceTxHash.toLowerCase()===String(step.source_tx_hash||"").toLowerCase(),"evm_cctp_receive_target");const message=abiBytes(data,0),attestation=abiBytes(data,1),sourceMessage=step.source_message,expectedMessage={sourceChain:action.sourceChain,destinationChain:scope.sourceChain,recipient:caller,amount:inputAmount},attestedParsed=verifyNoForwardMessage(message,expectedMessage),sourceParsed=verifyNoForwardMessage(sourceMessage,expectedMessage,{attested:false}),nonceStart=2+12*2,nonceEnd=2+44*2,finalityStart=2+144*2,finalityEnd=2+148*2;check(sourceMessage.slice(0,nonceStart).toLowerCase()===message.slice(0,nonceStart).toLowerCase()&&sourceMessage.slice(nonceEnd,finalityStart).toLowerCase()===message.slice(nonceEnd,finalityStart).toLowerCase()&&sourceMessage.slice(finalityEnd).toLowerCase()===message.slice(finalityEnd).toLowerCase(),"evm_cctp_receive_attested_drift");check(keccakHex(message).toLowerCase()===String(action.messageHash).toLowerCase(),"evm_cctp_receive_message_hash");check(keccakHex(sourceMessage).toLowerCase()===String(action.sourceMessageHash).toLowerCase()&&String(action.sourceMessageHash).toLowerCase()===String(step.source_message_hash||"").toLowerCase(),"evm_cctp_receive_source_hash");check(attestedParsed.nonce.toLowerCase()===String(action.nonce).toLowerCase(),"evm_cctp_receive_nonce");check(keccakHex(attestedParsed.body).toLowerCase()===String(action.messageBodyHash).toLowerCase()&&sourceParsed.body.toLowerCase()===attestedParsed.body.toLowerCase(),"evm_cctp_receive_body_hash");check(keccakHex(attestation).toLowerCase()===String(action.attestationHash).toLowerCase(),"evm_cctp_receive_attestation_hash");check(integer(action.burnAmountBase,"burn_amount",1)===inputAmount,"evm_cctp_receive_amount");destinationRecipient=caller;destinationDomain=PINS[scope.sourceChain].domain;
    ...
    check(canonical(receipt.approval)===canonical(derivedApproval),"receipt_approval");const allowed=step.provider==="raydium_clmm"?new Set([PROGRAMS.system,PROGRAMS.compute,PROGRAMS.token,PROGRAMS.token2022,PROGRAMS.ata,PROGRAMS.memo,PROGRAMS.raydium]):step.provider==="orca_whirlpool"?new Set([PROGRAMS.token,PROGRAMS.token2022,PROGRAMS.ata,PROGRAMS.orca]):step.provider==="circle_cctp"?new Set([PROGRAMS.token,PROGRAMS.ata,PROGRAMS.cctp]):step.provider==="paxos_usdg_layerzero_oft"?new Set([PROGRAMS.compute,PROGRAMS.paxos,PROGRAMS.token,PROGRAMS.token2022]):new Set();check(allowed.size>0&&rows.every((row)=>allowed.has(row.programId)),"solana_program_pins");
    ...
    const bps=Number(action.routeFeeBps??step.route_fee_bps),feeAmount=actionFeeAmount(action)??0n,transfers=[];for(const row of rows){const bytes=rawDataBytes(row,"solana_instruction");if([PROGRAMS.token,PROGRAMS.token2022].includes(row.programId)&&bytes.length===10&&bytes[0]===12)transfers.push({amount:bytes.readBigUInt64LE(1),row});}if(bps===1){check(transfers.length===1&&transfers[0].amount===feeAmount,"solana_fee_transfer");const keys=transfers[0].row.keys,feeMint=keys?.[1]?.pubkey,feeDestination=keys?.[2]?.pubkey,authority=keys?.[3]?.pubkey,expectedFeeAccount=action.feeRecipientAta??action.feeRecipientUsdcAta??action.feeRecipientTokenAccount,allowedMints=[receipt.spend.token.address_or_mint,receipt.receive.token.address_or_mint].filter((value)=>!String(value).startsWith("native:"));check(keys?.length>=4&&allowedMints.includes(feeMint)&&typeof expectedFeeAccount==="string"&&feeDestination===expectedFeeAccount&&authority===caller,"solana_fee_accounts");}else check(transfers.length===0,"solana_no_fee_transfer");if(receipt.action.kind==="solana_cctp")check(rows.length===(bps?3:1),"solana_cctp_instruction_count");let layerzeroNative=null;if(receipt.action.kind==="solana_layerzero"){check(rows.length===(bps?3:2),"solana_layerzero_instruction_count");const compute=rows[0],row=rows.at(-1),computeBytes=rawDataBytes(compute,"solana_instruction"),bytes=rawDataBytes(row,"solana_instruction"),keys=row.keys||[],native=integer(action.nativeFee,"native_fee",1),send=integer(action.sendAmount??action.inputAmount,"send_amount",1),minimum=integer(action.minimumOutput,"minimum_output",1),decodedRecipient=bytes.length===81?`0x${bytes.subarray(24,44).toString("hex")}`:null,exactFee=inputAmount*BigInt(bps)/10000n,feeMetadata=(bps===0&&feeAmount===0n&&send===inputAmount&&action.feeCollection==="none"&&action.feeRecipient===null&&action.feeRecipientTokenAccount===null)||(bps===1&&feeAmount===exactFee&&exactFee>0n&&send+feeAmount===inputAmount&&action.feeCollection==="atomic_exact_spl_transfer"&&action.feeRecipient===PINS.solana.feeRecipient&&action.feeRecipientTokenAccount===associatedTokenAddress(PINS.solana.feeRecipient,PINS.solana.USDG,PROGRAMS.token2022));check(compute.programId===PROGRAMS.compute&&Array.isArray(compute.keys)&&compute.keys.length===0&&computeBytes.length===5&&computeBytes[0]===2&&computeBytes.readUInt32LE(1)===400000,"solana_layerzero_compute_budget");check(action.direction==="solana_to_robinhood"&&[0,1].includes(bps)&&feeMetadata,"solana_layerzero_fee_binding");check(minimum===minimumOutput&&bytes.length===81&&bytes.subarray(0,8).toString("hex")==="66fb14bb414b0c45"&&bytes.readUInt32LE(8)===30416&&bytes.subarray(12,24).every((value)=>value===0)&&sameParty(decodedRecipient,step.recipient_wallet)&&bytes.readBigUInt64LE(44)===send&&bytes.readBigUInt64LE(52)===minimumOutput&&bytes.readUInt32LE(60)===0&&bytes[64]===0&&bytes.readBigUInt64LE(65)===native&&bytes.readBigUInt64LE(73)===0n,"solana_layerzero_arguments");check(keys.length>8&&keys[2]?.pubkey==="75sq4xbQ1snnWKmwZ9sDZjzaLxgiZedPcegXwCCQ2bnp"&&keys[4]?.pubkey==="EPcyVe3LTpcT2gtxiXbaJuJp2cSAWvkZYb6gkkgxsrJN"&&keys[5]?.pubkey===PINS.solana.USDG&&keys[6]?.pubkey===PROGRAMS.token2022&&keys[8]?.pubkey===PROGRAMS.paxos,"solana_layerzero_accounts");check(canonical(action.addressLookupTableAddresses??action.address_lookup_table_addresses)===canonical([PAXOS_SOLANA_LOOKUP_TABLE]),"solana_layerzero_lookup_table");layerzeroNative=native;destinationRecipient=decodedRecipient;}const nativeCap=layerzeroNative??(scope.sourceChain==="solana"&&scope.sourceToken==="SOL"?inputAmount:0n);check(canonical(receipt.native_value_cap)===canonical({maximum_base:String(nativeCap),unit:"lamports",per_transaction_base:[]}),"receipt_native_value_cap");destinationRecipient=destinationRecipient??action.recipient??(receipt.action.kind==="solana_dex"?caller:null);check(destinationRecipient===null||sameParty(destinationRecipient,step.recipient_wallet),"solana_recipient_wallet");if(receipt.action.kind==="solana_cctp"&&destinationDomain===null)destinationDomain=PINS[scope.destinationChain]?.domain??null;
  }
...
function verifyApprovalBundleBounds(bundle,quote,approval){
  ...
  const step=summary.steps[legacyReceive?0:index],spend=integer(receipt?.spend?.maximum_amount_base,"approval_spend",1),receive=integer(receipt?.receive?.minimum_amount_base,"approval_receive",1),stepExpectedInput=integer(step.expected_input_base,"approval_step_expected_input",1),stepMinimumInput=integer(step.minimum_input_base,"approval_step_minimum_input",1),stepMinimum=integer(step.minimum_output_base,"approval_step_output",1);
  if(index>0){const previous=workflow.steps[index-1],current=workflow.steps[index],attestationTransition=current?.provider==="circle_cctp_receive"&&previous?.status==="attestation_verified";if((previous?.status!=="receipt_verified"&&!attestationTransition)||integer(previous.actual_output_base,"approval_previous_output",1)!==spend)throw new Error("assetfare_plan_approval_bounds_binding_failed");}
  if((index===0&&spend>stepExpectedInput)||(index>0&&!legacyReceive&&spend<stepMinimumInput)||receive<stepMinimum||(index===0&&spend>BigInt(approval.maximum_input_base))||((legacyReceive||index===summary.step_count-1)&&receive<BigInt(approval.minimum_output_base)))throw new Error("assetfare_plan_approval_bounds_binding_failed");
...
async function runPlan(argv,{fetchImpl=fetch,stdout=process.stdout,nowMs}={}){
  ...
  const quote=parseV2Quote(rawQuote,intent,{nowMs:nowMs??Date.now()}),continuation=validateContinuationV3(quote.continuation_v3,quote,{requireUnexpired:true,nowMs:nowMs??Date.now()});

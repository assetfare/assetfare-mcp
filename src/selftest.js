...
try {
  ...
  if (health.status !== 200 || health.body?.version !== "0.4.4" || health.body?.server_signing !== false || health.body?.server_submission !== false) throw new Error("health contract mismatch");
  if (card.status !== 200 || card.body?.serverInfo?.version !== "0.4.4" || card.body?.tools?.length !== 22) throw new Error("server card contract mismatch");
...
console.log(JSON.stringify({ status: "pass", tool_count: names.length, health_version: "0.4.4", server_card_tools: 22, discovery_channel_cards: 3, has_submission_tool: false, provenance_validation: true, a2a_version_http_status: 400, a2a_patch_version_accepted: true, a2a_http_integration: true }));
await client.close();
await server.close();

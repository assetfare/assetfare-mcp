...
try {
  ...
  if (health.status !== 200 || health.body?.version !== "0.4.22" || health.body?.server_signing !== false || health.body?.server_submission !== false) throw new Error("health contract mismatch");
  if (card.status !== 200 || card.body?.serverInfo?.version !== "0.4.22" || card.body?.tools?.length !== 9 || card.body?.profile !== "v2") throw new Error("remote server card contract mismatch");

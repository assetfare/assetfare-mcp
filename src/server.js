...
function createServer(provenance = {}) {
  ...
  addTool(server, "assetfare_quote", "Get a fee-inclusive quote for the only launch corridor: Solana SOL to Base ETH. This does not create a session or transaction.", { amount_usd: z.number().int().min(250).max(1000) }, readonly(), ({ amount_usd }) => api("/v1/quote", { method: "POST", body: { from_chain: "solana", from_token: "SOL", to_chain: "base", to_token: "ETH", amount_usd } }));
  ...
  addTool(server, "assetfare_observe_cctp", "Verify Circle attestation and the forwarded Base USDC mint for an already-submitted burn. Read-only on chain, but advances the session record.", { access_token: accessToken, session_id: sessionId, burn_signature: z.string().min(64).max(128), idempotency_key: idempotencyKey }, stateful(true), ({ access_token, session_id, ...body }) => api(`/v1/session/${session_id}/observe-cctp`, { method: "POST", token: access_token, body }));

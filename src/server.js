...
const V2_SESSION_CREATE_DESCRIPTION = "Create one receipt-driven workflow. New callers must explicitly pass caller_approved=true, the exact approval_v3 selected as session, the same idempotency_key, a caller-local session token, and verification_context copied from the selected quote/public wallets. Example: create a strict session with matching approval/idempotency/context after selection. Every returned action is deeply verified and receives a self-verifying wallet handoff. Omitting strict context fails closed before an action is exposed. AssetFare never signs or submits.";
...
const remoteVerificationField={verification_context:v2RemoteVerificationContext.optional()};
v2SessionCreateIntent=z.object({ ...v2PrepareFields, approval_v3:v2SessionApproval.optional().describe("Strict Core 2.4.1 session quote-bound selection from the exact unexpired quote. Required for every new flow; omission remains legacy_advisory compatibility only."), session_token: v2SessionToken, idempotency_key: idempotencyKey,...remoteVerificationField }).strict();

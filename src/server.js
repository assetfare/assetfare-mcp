...
const idempotencyKey = z.string().min(8).max(128).describe("Caller-generated retry key, 8-128 characters. Reuse it only when retrying the same logical mutation; choose a new key for a new operation.");
...
const v2SessionToken = z.string().regex(/^[A-Za-z0-9_-]{43,128}$/).describe("Sensitive caller-owned v2 session capability from assetfare_v2_new_session_capability, 43-128 URL-safe characters. Never log it, and never substitute a private key or legacy access token.");
...
const v2PrepareFields = {
  caller_approved: z.literal(true).describe("Explicit confirmation that the caller approved this state-changing preparation or session creation. A quote alone is not approval."),
  ...v2QuoteFields,
  wallets: v2WalletMap,
...
function createServer(provenance = {}) {
  ...
  addTool(server, "assetfare_v2_new_session_capability", V2_NEW_SESSION_CAPABILITY_DESCRIPTION, emptyStrictInput, { readOnlyHint: true, destructiveHint: false, idempotentHint: false, openWorldHint: false }, async () => generateSessionCapability(), v2SessionCapabilityOutput);

...
const caps = { status:"capped_public_agent_release",public_api_enabled: true,chains:["arbitrum","base","polygon","robinhood","solana"],asset_endpoints:endpoints.map(([chain,token])=>({chain,token})),source_only_asset_endpoints:[{chain:"polygon",token:"USDC"}],source_only_routes:["polygon:USDC->base:USDC","polygon:USDC->arbitrum:USDC"],directed_conversion_routes:74,unsigned_route_plans_ready:74,server_signing: false, server_submission: false };

...
const quoteFor = (intent,{legacy=false}={}) => {
  const expected=intent.amount_usd-.01,minimum=intent.amount_usd-.02,small=.02/intent.amount_usd>=.01,routeName=`${intent.from_chain}:${intent.from_token}->${intent.to_chain}:${intent.to_token}`,contract=(legacy?LEGACY_ROUTES[routeName]:null)||DIRECT_ROUTES[routeName];
  if(!contract)throw new Error(`missing route fixture ${routeName}`);
  let expectedInput=1_000_000,minimumInput=1_000_000,cumulativeGuard=0;const rawSteps=[],summarySteps=[];
  for(const planned of contract.steps){const expectedOutput=Math.max(1,expectedInput-1000);if(Object.hasOwn(planned,"minimum_guard_bps"))cumulativeGuard+=planned.minimum_guard_bps;const minimumOutput=Object.hasOwn(planned,"minimum_guard_bps")?Number((BigInt(expectedOutput)*BigInt(10_000-cumulativeGuard)+9_999n)/10_000n):Math.max(1,minimumInput-2000),[fc,fa]=planned.from.split(":"),[tc,ta]=planned.to.split(":");let raw;
...
const falseDirectFetch=async(url,init={})=>{if(String(url).endsWith("/v2/capabilities"))return ok(caps);if(String(url).endsWith("/v2/status"))return ok(status);const value=quoteFor(JSON.parse(String(init.body)),{legacy:true});value.direct_route_summary.classification="direct_protocol_only";value.direct_route_summary.external_intent_protocol_used=false;value.direct_route_summary.provider_internal_dex_aggregation_possible=false;return ok(value);};

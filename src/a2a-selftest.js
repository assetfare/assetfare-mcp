...
const quoteFor = (intent) => ({ status: "capped_public_agent_release",intent:{from:`${intent.from_chain}:${intent.from_token}`,to:`${intent.to_chain}:${intent.to_token}`,amount_usd:intent.amount_usd,estimated_input_base:1},execution:{ supported: true, first_unsigned_action_supported:true, blocker:null }, risk: { server_signing: false, server_submission: false }, cost_summary:{scope:"token_path_only_network_gas_excluded",input_value_usd:intent.amount_usd,expected_receive_value_usd:.99,minimum_receive_value_usd:.98,expected_total_cost_usd:.01,maximum_total_cost_usd:.02,assetfare_service_fee:{bps:1},rankable_all_in:false,small_amount_warning:true},eta:{estimated_time_seconds:20,complete_route_estimate:true},offer: {expected_receive_amount:.99,estimated_min_receive_amount:.98,output_symbol:intent.to_token,expected_receive_usd:.99,estimated_min_receive_usd:.98,estimated_time_seconds:20,assetfare_fee_bps:1,fee_modeled_bps:1,fee_collectible_now:true,fee_collection_steps:[0],fee_collection:"only_on_eligible_successful_executor_step"},route:{route:`${intent.from_chain}:${intent.from_token}->${intent.to_chain}:${intent.to_token}`,steps:[{provider:"fixture"}],server_signing:false,server_submission:false}, caller_action_plan_handoff: executableHandoff(), caller_action_plan_handoff_v2: executableHandoffV2(), handoff_schema_version: 2 });

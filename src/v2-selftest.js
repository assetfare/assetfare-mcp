...
if(lockMetadata){assert.equal(lockMetadata.version, "1.3.1");assert.equal(lockMetadata.packages[""].version, "1.3.1");}
assert.equal(registryMetadata.version, "1.3.1");
if(bridgeRegistryMetadata)assert.equal(bridgeRegistryMetadata.version, "1.3.1");
...
try {
  ...
  assert.equal(card.serverInfo.version, "1.3.1");

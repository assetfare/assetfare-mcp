...
const EXPECTED_KEYWORDS = ["ai-agents", "route-quotes", "cross-chain", "cross-chain-swap", "bridge", "usdc", "usdc-bridge", "solana-to-base", "swap", "solana", "base", "arbitrum", "robinhood-chain", "polygon", "optimism", "mcp", "a2a", "openapi", "non-custodial"];
...
assert.match(packageMetadata.description, /Solana to Base USDC bridge/i);
...
assert.match(registryMetadata.description, /non-custodial bridge.*cross-chain swap.*Solana to Base USDC bridge.*76 routes.*unsigned only/i);
assert.doesNotMatch(registryMetadata.description, /best|leading|fastest|cheapest/i);
assert.match(readmeMetadata.slice(0, 2500), /Solana to Base USDC bridge/is);

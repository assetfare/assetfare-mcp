...
const EXPECTED_KEYWORDS = ["ai-agents", "route-quotes", "cross-chain", "cross-chain-swap", "bridge", "usdc-bridge", "cctp", "solana-to-base", "usdc", "swap", "solana", "base", "arbitrum", "robinhood-chain", "polygon", "optimism", "mcp", "a2a", "openapi", "non-custodial"];
...
assert.equal(packageMetadata.version, "0.4.12");
assert.equal(lockMetadata.version, "0.4.12");
assert.equal(lockMetadata.packages[""].version, "0.4.12");
assert.equal(registryMetadata.version, "0.4.12");
...
assert.match(registryMetadata.description, /bridge.*Solana to Base USDC.*unsigned.*never signs/i);
...
try {
  ...
  assert.equal(card.serverInfo.version, "0.4.12");

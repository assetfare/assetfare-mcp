...
const POLICY_VERSION="assetfare-caller-owned-execution-policy-v1";
const STATE_VERSION="assetfare-caller-owned-runner-state-v1";
const ADAPTER_VERSION="assetfare-caller-wallet-adapter-v1";
const PREPARATION_VERSION="assetfare-caller-wallet-preparation-v1";
const SUBMISSION_VERSION="assetfare-caller-wallet-submission-v1";
const CONFIRMATION_VERSION="assetfare-caller-wallet-confirmation-v1";
...
function validatePolicy(raw,capability,nowMs=Date.now()){
  const keys=["version","authorization_id","status","single_use","automatic_execution_authorized","authorization_expires_at","quote_id","session_id","route","wallets","maximum_input_base","minimum_final_output_base","minimum_action_remaining_seconds","maximum_runtime_seconds","evm","solana","custody"];
  ...
  safeInteger(raw.minimum_action_remaining_seconds,"minimum_remaining",30,170);safeInteger(raw.maximum_runtime_seconds,"maximum_runtime",60,3600);
...
function validateAdapter(adapter){
  ...
  for(const name of ["prepareEvmRequest","submitEvmRequest","confirmEvmTransaction","revokeEvmApproval","prepareSolanaAction","submitSolanaAction","confirmSolanaTransaction"])if(typeof adapter[name]!=="function")throw new Error("assetfare_runner_adapter_contract_invalid");
  return adapter;
}
...
function publicContext(policy,handoff){return {authorization_id:policy.authorization_id,policy_sha256:policy.sha256,route:policy.route,session_id:policy.session_id,action_id:handoff.action_id,step_index:handoff.step_index,assetfare_server_key_access:false,assetfare_server_signing:false,assetfare_server_submission:false};}
function validatePreparation(value,{family,operationId,bindingSha}){assertPublic(value,"preparation");if(value?.version!==PREPARATION_VERSION||value.family!==family||value.operation_id!==operationId||value.binding_sha256!==bindingSha||value.simulation_success!==true||typeof value.preparation_handle!=="string"||value.preparation_handle.length<8||value.preparation_handle.length>256||value.assetfare_server_key_access!==false||value.assetfare_server_signing!==false||value.assetfare_server_submission!==false)throw new Error("assetfare_runner_preparation_invalid");return value;}
function validateSubmission(value,{family,operationId,bindingSha}){assertPublic(value,"submission");const regex=family==="evm"?EVM_HASH:SOL_HASH;if(value?.version!==SUBMISSION_VERSION||value.family!==family||value.operation_id!==operationId||value.binding_sha256!==bindingSha||!regex.test(value.transaction_hash||"")||value.signed_by!=="caller_wallet_adapter"||value.submitted_by!=="caller_wallet_adapter"||value.assetfare_server_key_access!==false||value.assetfare_server_signing!==false||value.assetfare_server_submission!==false)throw new Error("assetfare_runner_submission_invalid");return value;}
function validateConfirmation(value,{family,operationId,hash,bindingSha}){assertPublic(value,"confirmation");if(value?.version!==CONFIRMATION_VERSION||value.family!==family||value.operation_id!==operationId||value.binding_sha256!==bindingSha||value.transaction_hash!==hash||value.success!==true||value.finalized!==true||value.assetfare_server_key_access!==false||value.assetfare_server_signing!==false||value.assetfare_server_submission!==false)throw new Error("assetfare_runner_confirmation_invalid");return value;}

async function executeEvm({handoff,adapter,policy,state,statePath,capability,clock}){
  ...
  try{
    for(let index=0;index<rows.length;index++){
      ...
      if(!item?.transaction_hash){
        if(Date.parse(handoff.expires_at)-clock()<15_000)throw new Error("assetfare_runner_handoff_expired_before_submission");
        const prepared=validatePreparation(await adapter.prepareEvmRequest({operation_id:operationId,request:structuredClone(request),request_index:index,maximum_gas_limit:policy.evm.gas_limit_caps[index].toString(),maximum_fee_per_gas_wei:maxFee.toString(),context:publicContext(policy,handoff)}),{family:"evm",operationId,bindingSha});
        ...
        const submitted=validateSubmission(await adapter.submitEvmRequest({operation_id:operationId,preparation_handle:prepared.preparation_handle,binding_sha256:bindingSha,context:publicContext(policy,handoff)}),{family:"evm",operationId,bindingSha});
        item={operation_id:operationId,binding_sha256:bindingSha,transaction_hash:submitted.transaction_hash,confirmed:false,maximum_native_spend_wei:maximum.toString(),protocol_value_wei:String(BigInt(request.value))};action.requests[index]=item;action.transaction_hashes=action.requests.filter(Boolean).map(value=>value.transaction_hash);action.status="submitted";state.totals.evm_transactions+=1;saveState(statePath,state,clock());
      }
      const confirmed=validateConfirmation(await adapter.confirmEvmTransaction({operation_id:item.operation_id,transaction_hash:item.transaction_hash,binding_sha256:item.binding_sha256,context:publicContext(policy,handoff)}),{family:"evm",operationId:item.operation_id,hash:item.transaction_hash,bindingSha:item.binding_sha256});
    ...
    const receipt=handoff.verified_bundle.unsigned_action.safety_receipt,approval=receipt?.approval,approvalDone=action.requests[0]?.confirmed===true,downstreamSubmitted=action.requests.slice(1).some(item=>item?.transaction_hash);
    if(approval?.required===true&&approvalDone&&!downstreamSubmitted&&policy.evm.allow_approval_revoke_on_failure){
      ...
      const recovered=assertPublic(await adapter.revokeEvmApproval({operation_id:operationId,chain_id:handoff.chain_id,owner:receipt.parties.caller,token:approval.token,target:approval.target,maximum_gas_limit:policy.evm.approval_recovery_gas_limit.toString(),maximum_fee_per_gas_wei:maxFee.toString(),binding_sha256:bindingSha,context:publicContext(policy,handoff)}),"recovery");
      if(recovered?.version!==CONFIRMATION_VERSION||recovered.family!=="evm"||recovered.operation_id!==operationId||recovered.binding_sha256!==bindingSha||!EVM_HASH.test(recovered.transaction_hash||"")||recovered.success!==true||recovered.finalized!==true||recovered.allowance_after_base!=="0"||recovered.assetfare_server_key_access!==false||recovered.assetfare_server_signing!==false||recovered.assetfare_server_submission!==false)throw new Error("assetfare_runner_approval_recovery_failed");
      const recoveryFee=bigint(recovered.actual_fee_wei,"adapter_evm_recovery_fee");if(recoveryFee>policy.evm.approval_recovery_gas_limit*maxFee)throw new Error("assetfare_runner_approval_recovery_cap_exceeded");state.totals.evm_transactions+=1;state.totals.evm_actual_fee_wei=(bigint(state.totals.evm_actual_fee_wei,"state_evm_actual")+recoveryFee).toString();state.totals.evm_actual_native_spend_wei=(bigint(state.totals.evm_actual_fee_wei,"state_evm_actual")+bigint(state.totals.evm_protocol_value_wei,"state_evm_protocol")).toString();action.recoveries=[...(action.recoveries||[]),{transaction_hash:recovered.transaction_hash,allowance_after_base:"0",actual_fee_wei:recoveryFee.toString()}];action.attempt+=1;action.requests=[];action.transaction_hashes=[];action.status="retryable_after_approval_recovery";saveState(statePath,state,clock());
    }
    throw error;
  }
...
async function executeSolana({handoff,adapter,policy,state,statePath,clock}){
  ...
  const action=actionState(state,handoff),bindingSha=digest(construction),operationId=`${cleanId(policy.authorization_id)}:${handoff.action_id}:${action.attempt}:0`;let item=action.requests[0];
  if(!item?.transaction_hash){
    if(Date.parse(handoff.expires_at)-clock()<15_000)throw new Error("assetfare_runner_handoff_expired_before_submission");
    const prepared=validatePreparation(await adapter.prepareSolanaAction({operation_id:operationId,construction:structuredClone(construction),maximum_fee_and_rent_lamports:policy.solana.maximum_total_fee_and_rent_lamports.toString(),context:publicContext(policy,handoff)}),{family:"solana",operationId,bindingSha});
    ...
    const submitted=validateSubmission(await adapter.submitSolanaAction({operation_id:operationId,preparation_handle:prepared.preparation_handle,binding_sha256:bindingSha,context:publicContext(policy,handoff)}),{family:"solana",operationId,bindingSha});

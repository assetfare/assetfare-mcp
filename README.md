...
Core 2.4.1 quotes also include strict `continuation_v3`. MCP 1.5.2 verifies the
...
`1.5.2` authorities. Do not create a duplicate `io.github.assetfare/*` listing
...
```bash
npx --yes --package=assetfare-mcp@1.5.2 assetfare-route-eval \
...
```bash
npx --yes --package=assetfare-mcp@1.5.2 assetfare-plan \
...
```bash
npx --yes --package=assetfare-mcp@1.5.2 assetfare-session \
...
```bash
npx --yes --package=assetfare-mcp@1.5.2 assetfare-agent-runner \
  --capability-file ./session-capability.json \

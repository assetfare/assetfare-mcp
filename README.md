...
Core 2.4.1 quotes also include strict `continuation_v3`. MCP 1.8.4 verifies the
...
`1.8.4` authorities. Do not create a duplicate `io.github.assetfare/*` listing
...
```bash
npx --yes --package=assetfare-mcp@1.8.4 assetfare-route-eval \
...
```bash
npx --yes --package=assetfare-mcp@1.8.4 assetfare-plan \
...
```bash
npx --yes --package=assetfare-mcp@1.8.4 assetfare-session \
...
```bash
npx --yes --package=assetfare-mcp@1.8.4 assetfare-adapter-conformance \
...
```bash
npx --yes --package=assetfare-mcp@1.8.4 assetfare-agent-runner \
  --capability-file ./session-capability.json \

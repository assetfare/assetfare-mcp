...
Core 2.4.1 quotes also include strict `continuation_v3`. MCP 1.10.0 verifies the
...
`1.10.0` authorities. Do not create a duplicate `io.github.assetfare/*` listing
...
```bash
npx --yes --package=assetfare-mcp@1.10.0 assetfare-route-eval \
...
```bash
npx --yes --package=assetfare-mcp@1.10.0 assetfare-plan \
...
```bash
npx --yes --package=assetfare-mcp@1.10.0 assetfare-session \
...
session header and never prints it. The mode-0600 capability also preserves the
strict approval, exact intent, public wallet map and direct-route summary needed
to apply the full first-action semantic verifier to every later `current_action`.
Each later EVM or Solana action receives a fresh self-verifying caller-wallet
...
- a mode-0600 policy matching
  [`caller-owned-execution-policy-v2`](./schemas/caller-owned-execution-policy-v2.json),
  containing only public wallets and explicit execution caps; and
- a local wallet adapter. The packaged
  [`standard-wallet-adapter`](./src/standard-wallet-adapter.js) already
  implements the AssetFare contract for a standard EIP-1193 provider and a
  Solana Wallet Standard wallet; the caller only wires its existing provider
  clients as shown in
  [`examples/caller-wallet-adapter.mjs`](./examples/caller-wallet-adapter.mjs).

Before funding it, validate the local adapter contract without invoking any
wallet, signer, RPC submit, or network request:

```bash
npx --yes --package=assetfare-mcp@1.10.0 assetfare-adapter-conformance \
...
```bash
npx --yes --package=assetfare-mcp@1.10.0 assetfare-agent-runner \
  --capability-file ./session-capability.json \
...
before each submission, and checks the policy, total runtime, and action expiry
again after simulation and immediately before every normal or recovery submit.
The v2 adapter must echo the exact execution-window hash and performs its own
final check before using the caller wallet. The runner invokes only the caller adapter, persists only public
...
The local adapter signs and submits through the caller's wallet/RPC. The
packaged reference adapter accepts standard provider objects, never key strings,
and stores only mode-0600 public operation metadata for idempotent retries. It returns
...
For a wallet that controls several EVM chains, `evmProvider` may be either one
EIP-1193 provider or an async `(chainId) => provider` resolver. The reference
adapter resolves the exact request chain before simulation, signing, submission
and confirmation, rejects a mismatched chain/account, and requires two EVM
...
AssetFare also exposes A2A Agent Card version 1.1.0 for agents that discover
...
token-generation operation. Every session operation also requires the same
hash-bound `verificationContext` copied from the exact selected quote, strict
approval and public-wallet map. The adapter validates that context before the
upstream request, deeply verifies every EVM/Solana `current_action`, and emits
a fresh self-verifying caller-wallet handoff. Each execution operation requires
an explicit `callerApproved:true` and the caller's public wallet addresses;
AssetFare never signs or submits, and only the caller's own submitted
transaction hashes are observed. A safe 409 response is preserved as structured
reapproval/recovery guidance; it never authorizes automatic requoting, replay,
